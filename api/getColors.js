...
export default async function handler(req, res) {
    ...
    try {
            ...
            parsedCSS.stylesheet.rules.forEach(rule => {
                if (rule.type === 'rule' || rule.type === 'media') {
                    rule.declarations?.forEach(declaration => {
                        ...
                        } else if (['color', 'background-color', 'border-color'].includes(declaration.property)) {
        ...
        // Capture pseudo-element colors via computed styles
        const styleElements = dom.window.document.querySelectorAll('*');
        styleElements.forEach((el) => {
            const styles = getComputedStyle(el);
            ['color', 'background-color', 'border-color'].forEach(prop => {
                const color = styles.getPropertyValue(prop);
                if (isValidColor(color)) colorList.add(color);
            });
        });
...
const colorToRgb = (color) => {
    ...
    return rgba ? { r: parseInt(rgba[1]), g: parseInt(rgba[2]), b: parseInt(rgba[3]) } : null;
};
